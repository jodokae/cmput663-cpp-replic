...
involved herein */
#define RESULT_STRING(s)x = PyString_FromString(l->s);if (!x) goto failed;PyDict_SetItemString(result, #s, x);Py_XDECREF(x)
#define RESULT_INT(i)x = PyInt_FromLong(l->i);if (!x) goto failed;PyDict_SetItemString(result, #i, x);Py_XDECREF(x)
/* Numeric information */
