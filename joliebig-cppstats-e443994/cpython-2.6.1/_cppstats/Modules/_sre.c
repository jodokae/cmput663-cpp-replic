...
#define MARK_PUSH(lastmark) do if (lastmark > 0) { i = lastmark; /* ctx->lastmark may change if reallocated */ DATA_STACK_PUSH(state, state->mark, (i+1)*sizeof(void*)); } while (0)
...
#define DO_JUMP(jumpvalue, jumplabel, nextpattern) DATA_ALLOC(SRE_MATCH_CONTEXT, nextctx); nextctx->last_ctx_pos = ctx_pos; nextctx->jump = jumpvalue; nextctx->pattern = nextpattern; ctx_pos = alloc_pos; ctx = nextctx; goto entrance; jumplabel: while (0) /* gcc doesn't like labels at end of scopes */
typedef struct {
