...
#define SPLIT_APPEND(data, left, right) str = PyString_FromStringAndSize((data) + (left), (right) - (left)); if (str == NULL) goto onError; if (PyList_Append(list, str)) { Py_DECREF(str); goto onError; } else Py_DECREF(str);
#define SPLIT_ADD(data, left, right) { str = PyString_FromStringAndSize((data) + (left), (right) - (left)); if (str == NULL) goto onError; if (count < MAX_PREALLOC) { PyList_SET_ITEM(list, count, str); } else { if (PyList_Append(list, str)) { Py_DECREF(str); goto onError; } else Py_DECREF(str); } count++; }
