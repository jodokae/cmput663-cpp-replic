...
#define Py_STRING_MATCH(target, offset, pattern, length) (target[offset] == pattern[0] && target[offset+length-1] == pattern[length-1] && !memcmp(target+offset+1, pattern+1, length-2) )
...
#define SPLIT_APPEND(data, left, right) str = PyByteArray_FromStringAndSize((data) + (left), (right) - (left)); if (str == NULL) goto onError; if (PyList_Append(list, str)) { Py_DECREF(str); goto onError; } else Py_DECREF(str);
#define SPLIT_ADD(data, left, right) { str = PyByteArray_FromStringAndSize((data) + (left), (right) - (left)); if (str == NULL) goto onError; if (count < MAX_PREALLOC) { PyList_SET_ITEM(list, count, str); } else { if (PyList_Append(list, str)) { Py_DECREF(str); goto onError; } else Py_DECREF(str); } count++; }
...
(ssizeargfunc)bytes_getitem,
0,
(ssizeobjargproc)bytes_setitem,
0,
(objobjproc)bytes_contains,
(binaryfunc)bytes_iconcat,
(ssizeargfunc)bytes_irepeat,
};
...
0,
(traverseproc)bytesiter_traverse,
0,
0,
0,
PyObject_SelfIter,
