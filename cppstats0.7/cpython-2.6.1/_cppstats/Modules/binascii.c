...
out_data = (unsigned char *)PyString_AsString(rv);
#define INBYTE(b) do { if ( --in_len < 0 ) { PyErr_SetString(Incomplete, ""); Py_DECREF(rv); return NULL; } b = *in_data++; } while(0)
#define OUTBYTE(b) do { if ( --out_len_left < 0 ) { if ( out_len > PY_SSIZE_T_MAX / 2) return PyErr_NoMemory(); _PyString_Resize(&rv, 2*out_len); if ( rv == NULL ) return NULL; out_data = (unsigned char *)PyString_AsString(rv) + out_len; out_len_left = out_len-1; out_len = out_len * 2; } *out_data++ = b; } while(0)
INBYTE(in_byte);
if (in_byte == RUNCHAR) {
INBYTE(in_repeat);
if (in_repeat != 0) {
