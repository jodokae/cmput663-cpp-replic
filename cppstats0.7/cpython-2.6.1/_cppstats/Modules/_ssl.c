...
#include "pythread.h"
#define PySSL_BEGIN_ALLOW_THREADS { PyThreadState *_save = NULL; if (_ssl_locks_count>0) {_save = PyEval_SaveThread();}
...
self = PyObject_New(PySSLObject, &PySSL_Type);
if (self == NULL)
return NULL;
...
self->ctx = SSL_CTX_new(SSLv3_method());
else if (proto_version == PY_SSL_VERSION_SSL2)
self->ctx = SSL_CTX_new(SSLv2_method());
