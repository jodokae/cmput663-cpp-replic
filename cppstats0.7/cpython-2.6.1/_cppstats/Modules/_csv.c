...
#endif
#define IS_BASESTRING(o) PyObject_TypeCheck(o, &PyBaseString_Type)
...
Py_XINCREF(strict);
if (dialect != NULL) {
#define DIALECT_GETATTR(v, n) if (v == NULL) v = PyObject_GetAttrString(dialect, n)
...
#define DIASET(meth, name, target, src, dflt) if (meth(name, target, src, dflt)) goto err
...
int i, rec_len;
char *lineterm;
#define ADDCH(c) do {if (copy_phase) self->rec[rec_len] = c;rec_len++;} while(0)
