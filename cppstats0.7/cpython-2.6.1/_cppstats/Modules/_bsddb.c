...
#define MYDB_BEGIN_BLOCK_THREADS PyGILState_STATE __savestate = PyGILState_Ensure();
#define MYDB_END_BLOCK_THREADS PyGILState_Release(__savestate);
#else
static PyInterpreterState* _db_interpreterState = NULL;
#define MYDB_BEGIN_BLOCK_THREADS { PyThreadState* prevState; PyThreadState* newState; PyEval_AcquireLock(); newState = PyThreadState_New(_db_interpreterState); prevState = PyThreadState_Swap(newState);
#define MYDB_END_BLOCK_THREADS newState = PyThreadState_Swap(prevState); PyThreadState_Clear(newState); PyEval_ReleaseLock(); PyThreadState_Delete(newState); }
#endif
#else
...
static PyObject* DBIncompleteError;
#endif
...
#if !defined(staticforward)
#define staticforward static
#endif
#if !defined(statichere)
...
#endif
#define INSERT_IN_DOUBLE_LINKED_LIST(backlink,object) { object->sibling_next=backlink; object->sibling_prev_p=&(backlink); backlink=object; if (object->sibling_next) { object->sibling_next->sibling_prev_p=&(object->sibling_next); } }
#define EXTRACT_FROM_DOUBLE_LINKED_LIST(object) { if (object->sibling_next) { object->sibling_next->sibling_prev_p=object->sibling_prev_p; } *(object->sibling_prev_p)=object->sibling_next; }
#define EXTRACT_FROM_DOUBLE_LINKED_LIST_MAYBE_NULL(object) { if (object->sibling_next) { object->sibling_next->sibling_prev_p=object->sibling_prev_p; } if (object->sibling_prev_p) { *(object->sibling_prev_p)=object->sibling_next; } }
#define INSERT_IN_DOUBLE_LINKED_LIST_TXN(backlink,object) { object->sibling_next_txn=backlink; object->sibling_prev_p_txn=&(backlink); backlink=object; if (object->sibling_next_txn) { object->sibling_next_txn->sibling_prev_p_txn= &(object->sibling_next_txn); } }
#define EXTRACT_FROM_DOUBLE_LINKED_LIST_TXN(object) { if (object->sibling_next_txn) { object->sibling_next_txn->sibling_prev_p_txn= object->sibling_prev_p_txn; } *(object->sibling_prev_p_txn)=object->sibling_next_txn; }
#define RETURN_IF_ERR() if (makeDBError(err)) { return NULL; }
#define RETURN_NONE() Py_INCREF(Py_None); return Py_None;
#define _CHECK_OBJECT_NOT_CLOSED(nonNull, pyErrObj, name) if ((nonNull) == NULL) { PyObject *errTuple = NULL; errTuple = Py_BuildValue("(is)", 0, #name " object has been closed"); if (errTuple) { PyErr_SetObject((pyErrObj), errTuple); Py_DECREF(errTuple); } return NULL; }
...
#if (DBVER >= 43)
#define CHECK_SEQUENCE_NOT_CLOSED(curs) _CHECK_OBJECT_NOT_CLOSED(curs->sequence, DBError, DBSequence)
#endif
#define CHECK_DBFLAG(mydb, flag) (((mydb)->flags & (flag)) || (((mydb)->myenvobj != NULL) && ((mydb)->myenvobj->flags & (flag))))
#define CLEAR_DBT(dbt) (memset(&(dbt), 0, sizeof(dbt)))
#define FREE_DBT(dbt) if ((dbt.flags & (DB_DBT_MALLOC|DB_DBT_REALLOC)) && dbt.data != NULL) { free(dbt.data); dbt.data = NULL; }
...
#endif
case DB_KEYEMPTY:
errObj = DBKeyEmptyError;
break;
case DB_KEYEXIST:
errObj = DBKeyExistError;
break;
case DB_LOCK_DEADLOCK:
errObj = DBLockDeadlockError;
break;
case DB_LOCK_NOTGRANTED:
errObj = DBLockNotGrantedError;
break;
case DB_NOTFOUND:
errObj = DBNotFoundError;
break;
case DB_OLD_VERSION:
errObj = DBOldVersionError;
break;
case DB_RUNRECOVERY:
errObj = DBRunRecoveryError;
break;
case DB_VERIFY_BAD:
errObj = DBVerifyBadError;
break;
case DB_NOSERVER:
errObj = DBNoServerError;
break;
case DB_NOSERVER_HOME:
errObj = DBNoServerHomeError;
break;
case DB_NOSERVER_ID:
errObj = DBNoServerIDError;
break;
case DB_PAGE_NOTFOUND:
errObj = DBPageNotFoundError;
break;
case DB_SECONDARY_BAD:
errObj = DBSecondaryBadError;
break;
case DB_BUFFER_SMALL:
errObj = DBNoMemoryError;
break;
#if (DBVER >= 43)
case ENOMEM:
errObj = PyExc_MemoryError;
break;
#endif
case EINVAL:
errObj = DBInvalidArgError;
break;
case EACCES:
errObj = DBAccessError;
break;
case ENOSPC:
errObj = DBNoSpaceError;
break;
case EAGAIN:
errObj = DBAgainError;
break;
...
break;
case EEXIST:
errObj = DBFileExistsError;
break;
case ENOENT:
errObj = DBNoSuchFileError;
break;
case EPERM :
errObj = DBPermissionsError;
break;
#if (DBVER >= 42)
case DB_REP_HANDLE_DEAD :
errObj = DBRepHandleDeadError;
break;
#endif
case DB_REP_UNAVAIL :
errObj = DBRepUnavailError;
break;
default:
errObj = DBError;
break;
...
secKey->flags = DB_DBT_APPMALLOC;
secKey->data = malloc(size);
if (secKey->data) {
...
0,
0,
offsetof(DBObject, in_weakreflist),
0,
0,
DB_methods,
0,
...
0,
0,
offsetof(DBCursorObject, in_weakreflist),
0,
0,
DBCursor_methods,
0,
...
0,
0,
offsetof(DBEnvObject, in_weakreflist),
0,
0,
DBEnv_methods,
0,
...
0,
0,
offsetof(DBTxnObject, in_weakreflist),
0,
0,
DBTxn_methods,
0,
...
0,
0,
offsetof(DBLockObject, in_weakreflist),
};
#if (DBVER >= 43)
...
#endif
"DBSequence",
sizeof(DBSequenceObject),
0,
(destructor)DBSequence_dealloc,
0,
0,
0,
0,
...
0,
0,
offsetof(DBSequenceObject, in_weakreflist),
0,
0,
DBSequence_methods,
0,
...
#if defined(PYBSDDB_STANDALONE)
#define PYBSDDB_EXCEPTION_BASE "bsddb3.db."
#else
#define PYBSDDB_EXCEPTION_BASE "bsddb.db."
#endif
#define MAKE_EX(name) name = PyErr_NewException(PYBSDDB_EXCEPTION_BASE #name, DBError, NULL); PyDict_SetItemString(d, #name, name)
DBError = NULL;
MAKE_EX(DBError);
#if (PY_VERSION_HEX < 0x03000000)
...
#define MAKE_EX2(name) name = PyErr_NewException(PYBSDDB_EXCEPTION_BASE #name, bases, NULL); PyDict_SetItemString(d, #name, name)
