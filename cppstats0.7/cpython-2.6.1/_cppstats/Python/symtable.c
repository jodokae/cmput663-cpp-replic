...
#define VISIT_SEQ(ST, TYPE, SEQ) { int i; asdl_seq *seq = (SEQ); for (i = 0; i < asdl_seq_LEN(seq); i++) { TYPE ##_ty elt = (TYPE ##_ty)asdl_seq_GET(seq, i); if (!symtable_visit_ ##TYPE((ST), elt)) return 0; } }
#define VISIT_SEQ_IN_BLOCK(ST, TYPE, SEQ, S) { int i; asdl_seq *seq = (SEQ); for (i = 0; i < asdl_seq_LEN(seq); i++) { TYPE ##_ty elt = (TYPE ##_ty)asdl_seq_GET(seq, i); if (!symtable_visit_ ##TYPE((ST), elt)) { symtable_exit_block((ST), (S)); return 0; } } }
#define VISIT_SEQ_TAIL(ST, TYPE, SEQ, START) { int i; asdl_seq *seq = (SEQ); for (i = (START); i < asdl_seq_LEN(seq); i++) { TYPE ##_ty elt = (TYPE ##_ty)asdl_seq_GET(seq, i); if (!symtable_visit_ ##TYPE((ST), elt)) return 0; } }
#define VISIT_SEQ_TAIL_IN_BLOCK(ST, TYPE, SEQ, START, S) { int i; asdl_seq *seq = (SEQ); for (i = (START); i < asdl_seq_LEN(seq); i++) { TYPE ##_ty elt = (TYPE ##_ty)asdl_seq_GET(seq, i); if (!symtable_visit_ ##TYPE((ST), elt)) { symtable_exit_block((ST), (S)); return 0; } } }
static int
