...
#define DECLPROC(dll, result, name, args)typedef result (*__PROC__##name) args;result (*name)args = (__PROC__##name)GetProcAddress(dll, #name)
#define DECLVAR(dll, type, name)type *name = (type*)GetProcAddress(dll, #name)
...
#if defined(USE_OTHER_PYTHON_VERSIONS)
