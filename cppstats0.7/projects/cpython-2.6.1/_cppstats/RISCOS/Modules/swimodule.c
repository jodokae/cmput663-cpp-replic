...
return 0;
fail:
PyErr_SetString(PyExc_TypeError,"block slice must be integer list");
return -1;
}
static PySequenceMethods block_as_sequence= {
(inquiry)block_len,
(binaryfunc)block_concat,
(ssizeargfunc)block_repeat,
(ssizeargfunc)block_item,
(ssizessizeargfunc)block_slice,
(ssizeobjargproc)block_ass_item,
(ssizessizeobjargproc)block_ass_slice,
};
...
0,
"block",
sizeof(PyBlockObject),
0,
(destructor)PyBlock_Dealloc,
0,
(getattrfunc)PyBlock_GetAttr,
0,
0,
0,
0,
...
switch(*fmt) {
case 'i':
if(!PyArg_Parse(v,"i",&r.r[rno])) return NULL;
break;
case 's':
if(!PyArg_Parse(v,"s",(char**)(&r.r[rno]))) return NULL;
break;
case 'b':
if(!PyArg_Parse(v,"O",(PyObject**)&ao)) return NULL;
...
break;
default:
return swi_error("Odd format character");
}
rno++;
}
swicall:
e=(os_error*)_kernel_swi_c(swino,&r,&r,&carry);
if(e) return swi_oserror();
if(*fmt==0) {
Py_INCREF(Py_None);
return Py_None;
...
switch(*fmt) {
case 'i':
v=PyInt_FromLong((long)r.r[rno++]);
break;
case 's':
v=PyString_FromString((char*)(r.r[rno++]));
break;
case '.':
rno++;
continue;
case '*':
v=PyInt_FromLong((long)carry);
break;
}
if(!v) goto fail;
if(n==1) return v;
