...
#else
#define STRING_CONV_FUNC (self->returns_unicode ? conv_string_to_unicode : conv_string_to_utf8)
#endif
static PyObject*
...
#define RC_HANDLER(RC, NAME, PARAMS, INIT, PARAM_FORMAT, CONVERSION, RETURN, GETUSERDATA) static RC my_##NAME##Handler PARAMS {xmlparseobject *self = GETUSERDATA ; PyObject *args = NULL; PyObject *rv = NULL; INIT if (have_handler(self, NAME)) { if (flush_character_buffer(self) < 0) return RETURN; args = Py_BuildValue PARAM_FORMAT ;if (!args) { flag_error(self); return RETURN;} self->in_callback = 1; rv = call_with_frame(getcode(NAME,#NAME,__LINE__), self->handlers[NAME], args, self); self->in_callback = 0; Py_DECREF(args); if (rv == NULL) { flag_error(self); return RETURN; } CONVERSION Py_DECREF(rv); } return RETURN; }
#define VOID_HANDLER(NAME, PARAMS, PARAM_FORMAT) RC_HANDLER(void, NAME, PARAMS, ;, PARAM_FORMAT, ;, ;,(xmlparseobject *)userData)
#define INT_HANDLER(NAME, PARAMS, PARAM_FORMAT)RC_HANDLER(int, NAME, PARAMS, int rc=0;, PARAM_FORMAT, rc = PyInt_AsLong(rv);, rc, (xmlparseobject *)userData)
...
#define APPEND(list, str) do { PyObject *o = PyString_FromString(str); if (o != NULL) PyList_Append(list, o); Py_XDECREF(o); } while (0)
...
"pyexpat.xmlparser",
sizeof(xmlparseobject) + PyGC_HEAD_SIZE,
0,
(destructor)xmlparse_dealloc,
(printfunc)0,
(getattrfunc)xmlparse_getattr,
(setattrfunc)xmlparse_setattr,
(cmpfunc)0,
(reprfunc)0,
0,
0,
