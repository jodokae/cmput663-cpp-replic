...
#endif
#define LASTMARK_SAVE() do { ctx->lastmark = state->lastmark; ctx->lastindex = state->lastindex; } while (0)
#define LASTMARK_RESTORE() do { state->lastmark = ctx->lastmark; state->lastindex = ctx->lastindex; } while (0)
...
#define SFY(x) #x
#define DATA_STACK_ALLOC(state, type, ptr) do { alloc_pos = state->data_stack_base; TRACE(("allocating %s in %d (%d)\n", SFY(type), alloc_pos, sizeof(type))); if (state->data_stack_size < alloc_pos+sizeof(type)) { int j = data_stack_grow(state, sizeof(type)); if (j < 0) return j; if (ctx_pos != -1) DATA_STACK_LOOKUP_AT(state, SRE_MATCH_CONTEXT, ctx, ctx_pos); } ptr = (type*)(state->data_stack+alloc_pos); state->data_stack_base += sizeof(type); } while (0)
#define DATA_STACK_LOOKUP_AT(state, type, ptr, pos) do { TRACE(("looking up %s at %d\n", SFY(type), pos)); ptr = (type*)(state->data_stack+pos); } while (0)
#define DATA_STACK_PUSH(state, data, size) do { TRACE(("copy data in %p to %d (%d)\n", data, state->data_stack_base, size)); if (state->data_stack_size < state->data_stack_base+size) { int j = data_stack_grow(state, size); if (j < 0) return j; if (ctx_pos != -1) DATA_STACK_LOOKUP_AT(state, SRE_MATCH_CONTEXT, ctx, ctx_pos); } memcpy(state->data_stack+state->data_stack_base, data, size); state->data_stack_base += size; } while (0)
#define DATA_STACK_POP(state, data, size, discard) do { TRACE(("copy data to %p from %d (%d)\n", data, state->data_stack_base-size, size)); memcpy(data, state->data_stack+state->data_stack_base-size, size); if (discard) state->data_stack_base -= size; } while (0)
#define DATA_STACK_POP_DISCARD(state, size) do { TRACE(("discard data from %d (%d)\n", state->data_stack_base-size, size)); state->data_stack_base -= size; } while(0)
...
#define MARK_PUSH(lastmark) do if (lastmark > 0) { i = lastmark; DATA_STACK_PUSH(state, state->mark, (i+1)*sizeof(void*)); } while (0)
#define MARK_POP(lastmark) do if (lastmark > 0) { DATA_STACK_POP(state, state->mark, (lastmark+1)*sizeof(void*), 1); } while (0)
#define MARK_POP_KEEP(lastmark) do if (lastmark > 0) { DATA_STACK_POP(state, state->mark, (lastmark+1)*sizeof(void*), 0); } while (0)
#define MARK_POP_DISCARD(lastmark) do if (lastmark > 0) { DATA_STACK_POP_DISCARD(state, (lastmark+1)*sizeof(void*)); } while (0)
#define JUMP_NONE 0
...
#define DO_JUMP(jumpvalue, jumplabel, nextpattern) DATA_ALLOC(SRE_MATCH_CONTEXT, nextctx); nextctx->last_ctx_pos = ctx_pos; nextctx->jump = jumpvalue; nextctx->pattern = nextpattern; ctx_pos = alloc_pos; ctx = nextctx; goto entrance; jumplabel: while (0)
typedef struct {
...
#define GET_OP do { VTRACE(("%p: ", code)); if (code >= end) FAIL; op = *code++; VTRACE(("%lu (op)\n", (unsigned long)op)); } while (0)
#define GET_ARG do { VTRACE(("%p= ", code)); if (code >= end) FAIL; arg = *code++; VTRACE(("%lu (arg)\n", (unsigned long)arg)); } while (0)
#define GET_SKIP_ADJ(adj) do { VTRACE(("%p= ", code)); if (code >= end) FAIL; skip = *code; VTRACE(("%lu (skip to %p)\n", (unsigned long)skip, code+skip)); if (code+skip-adj < code || code+skip-adj > end)FAIL; code++; } while (0)
