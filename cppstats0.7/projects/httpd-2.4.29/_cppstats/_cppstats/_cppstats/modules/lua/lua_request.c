...
#if !defined(MODLUA_MAX_REG_MATCH)
...
switch (r->proxyreq) {
case PROXYREQ_NONE:
return "PROXYREQ_NONE";
case PROXYREQ_PROXY:
return "PROXYREQ_PROXY";
case PROXYREQ_REVERSE:
return "PROXYREQ_REVERSE";
case PROXYREQ_RESPONSE:
return "PROXYREQ_RESPONSE";
default:
return NULL;
}
}
...
#define APLUA_REQ_TRACE(lev) static int req_trace##lev(lua_State *L) { return req_log_at(L, APLOG_TRACE##lev); }
APLUA_REQ_TRACE(1)
APLUA_REQ_TRACE(2)
APLUA_REQ_TRACE(3)
APLUA_REQ_TRACE(4)
...
luaL_setfuncs_compat(L, request_methods);
lua_pop(L, 2);
luaL_newmetatable(L, "Apache2.Connection");
lua_pushvalue(L, -1);
lua_setfield(L, -2, "__index");
luaL_setfuncs_compat(L, connection_methods);
lua_pop(L, 2);
luaL_newmetatable(L, "Apache2.Server");
lua_pushvalue(L, -1);
lua_setfield(L, -2, "__index");
luaL_setfuncs_compat(L, server_methods);
lua_pop(L, 2);
}
