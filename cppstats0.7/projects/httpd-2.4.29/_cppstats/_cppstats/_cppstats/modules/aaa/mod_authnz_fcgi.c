...
#if !defined(NON200_RESPONSE_BUF_LEN)
...
sizeof(abrb), 0);
ap_fcgi_fill_in_request_body(&brb, role, 0 );
...
if (r->status == HTTP_OK) {
apr_table_t *vars = apr_table_make(temp_pool,
10);
...
char rspbuf[NON200_RESPONSE_BUF_LEN + 1];
...
rspbuf[rspbuflen] = '\0';
ap_custom_response(r, r->status, rspbuf);
}
}
