...
#if defined(DEBUG_INCLUDE)
#define TYPE_TOKEN(token, ttype) do { (token)->type = ttype; (token)->s = #ttype; } while(0)
#define CREATE_NODE(ctx, name) do { (name) = apr_palloc((ctx)->dpool, sizeof(*(name))); (name)->parent = (name)->left = (name)->right = NULL; (name)->done = 0; (name)->dump_done = 0; } while(0)
