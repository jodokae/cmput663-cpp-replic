...
#if !defined(REWRITELOG_DISABLED)
...
#if !defined(REWRITE_MAX_TXT_MAP_LINE)
#define REWRITE_MAX_TXT_MAP_LINE 1024
#endif
#if !defined(REWRITE_PRG_MAP_BUF)
#define REWRITE_PRG_MAP_BUF 1024
#endif
#define LEFT_CURLY '{'
#define RIGHT_CURLY '}'
#define SMALL_EXPANSION 5
#define subreq_ok(r) (!r->main || (r->main->uri && r->uri && strcmp(r->main->uri, r->uri)))
#if !defined(REWRITE_MAX_ROUNDS)
...
*sqs = 1;
return 8;
} else if (!strncasecmp(uri, "2://", 4)) {
*sqs = 1;
return 5;
} else if (!strncasecmp(uri, "2c://", 5)) {
*sqs = 1;
return 6;
}
break;
...
case 'w':
case 'W':
if (!strncasecmp(uri, "s://", 4)) {
*sqs = 1;
return 5;
} else if (!strncasecmp(uri, "ss://", 5)) {
*sqs = 1;
return 6;
}
break;
...
switch (a2[1]) {
case 'f':
newcond->ptype = CONDPAT_FILE_EXISTS;
break;
case 's':
newcond->ptype = CONDPAT_FILE_SIZE;
break;
case 'd':
newcond->ptype = CONDPAT_FILE_DIR;
break;
case 'x':
newcond->ptype = CONDPAT_FILE_XBIT;
break;
case 'h':
...
break;
case 'U':
newcond->ptype = CONDPAT_LU_URL;
break;
case 'F':
newcond->ptype = CONDPAT_LU_FILE;
break;
}
} else if (a2[3]) {
switch (a2[1]) {
