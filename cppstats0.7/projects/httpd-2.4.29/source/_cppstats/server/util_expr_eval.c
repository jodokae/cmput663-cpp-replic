...
#define DUMP_E_E(op, e1, e2) do { ap_log_error(MARK,"%*s%s: %pp %pp", indent, " ", op, e1, e2); if (e1) expr_dump_tree(e1, s, loglevel, indent + 2); if (e2) expr_dump_tree(e2, s, loglevel, indent + 2); } while (0)
#define DUMP_S_E(op, s1, e1) do { ap_log_error(MARK,"%*s%s: '%s' %pp", indent, " ", op, (char *)s1, e1); if (e1) expr_dump_tree(e1, s, loglevel, indent + 2); } while (0)
#define DUMP_S_P(op, s1, p1) ap_log_error(MARK,"%*s%s: '%s' %pp", indent, " ", op, (char *)s1, p1);
#define DUMP_P_P(op, p1, p2) ap_log_error(MARK,"%*s%s: %pp %pp", indent, " ", op, p1, p2);
#define DUMP_S_S(op, s1, s2) ap_log_error(MARK,"%*s%s: '%s' '%s'", indent, " ", op, (char *)s1, (char *)s2)
#define DUMP_P(op, p1) ap_log_error(MARK,"%*s%s: %pp", indent, " ", op, p1);
#define DUMP_IP(op, p1) ap_log_error(MARK,"%*s%s: %d", indent, " ", op, *(int *)p1);
#define DUMP_S(op, s1) ap_log_error(MARK,"%*s%s: '%s'", indent, " ", op, (char *)s1)
