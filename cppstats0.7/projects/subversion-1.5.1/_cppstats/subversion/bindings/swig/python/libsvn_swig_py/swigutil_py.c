...
#endif
#if defined(ACQUIRE_PYTHON_LOCK)
...
goto finished;
i = 0;
#define append(item) if (PyTuple_SetItem(args, i++, item) == 0) item = NULL; else goto finished;
append(apr_err_ob);
append(message_ob);
append(file_ob);
append(line_ob);
...
#define DECLARE_SWIG_CONSTRUCTOR(type, dup) static PyObject *make_ob_##type(void *value) { apr_pool_t *new_pool = svn_pool_create(application_pool); PyObject *new_py_pool = svn_swig_NewPointerObj(new_pool, svn_swig_TypeQuery("apr_pool_t *"), application_py_pool, NULL); svn_##type##_t *new_value = dup(value, new_pool); PyObject *obj = svn_swig_NewPointerObjString(new_value, "svn_" #type "_t *", new_py_pool); Py_XDECREF(new_py_pool); return obj; }
