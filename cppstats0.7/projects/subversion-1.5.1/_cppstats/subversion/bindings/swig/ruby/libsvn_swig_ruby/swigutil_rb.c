...
#if !defined(RE_OPTION_IGNORECASE)
#if defined(ONIG_OPTION_IGNORECASE)
...
#define DEFINE_DUP_BASE(type, dup_func, type_prefix) static VALUE c2r_ ##type ##_dup(void *type, void *ctx) { apr_pool_t *pool; VALUE rb_pool; svn_ ##type ##_t *copied_item; VALUE rb_copied_item; if (!type) return Qnil; svn_swig_rb_get_pool(0, (VALUE *)0, 0, &rb_pool, &pool); copied_item = svn_ ##dup_func((type_prefix svn_ ##type ##_t *)type, pool); rb_copied_item = c2r_swig_type((void *)copied_item, (void *)"svn_" #type "_t *"); rb_set_pool(rb_copied_item, rb_pool); return rb_copied_item; }
#define DEFINE_DUP_BASE_WITH_CONVENIENCE(type, dup_func, type_prefix) DEFINE_DUP_BASE(type, dup_func, type_prefix) static VALUE c2r_ ##type ##__dup(type_prefix svn_ ##type ##_t *type) { void *void_type; void_type = (void *)type; return c2r_ ##type ##_dup(void_type, NULL); }
#define DEFINE_DUP_WITH_FUNCTION_NAME(type, dup_func) DEFINE_DUP_BASE_WITH_CONVENIENCE(type, dup_func, const)
#define DEFINE_DUP(type) DEFINE_DUP_WITH_FUNCTION_NAME(type, type ##_dup)
#define DEFINE_DUP_NO_CONVENIENCE_WITH_FUNCTION_NAME(type, dup_func) DEFINE_DUP_BASE(type, dup_func, const)
#define DEFINE_DUP_NO_CONVENIENCE(type) DEFINE_DUP_NO_CONVENIENCE_WITH_FUNCTION_NAME(type, type ##_dup)
#define DEFINE_DUP_NO_CONST_WITH_FUNCTION_NAME(type, dup_func) DEFINE_DUP_BASE_WITH_CONVENIENCE(type, dup_func,)
#define DEFINE_DUP_NO_CONST(type) DEFINE_DUP_NO_CONST_WITH_FUNCTION_NAME(type, type ##_dup)
#define DEFINE_DUP_NO_CONST_NO_CONVENIENCE_WITH_FUNCTION_NAME(type, dup_func) DEFINE_DUP_BASE(type, dup_func,)
#define DEFINE_DUP_NO_CONST_NO_CONVENIENCE(type) DEFINE_DUP_NO_CONST_NO_CONVENIENCE_WITH_FUNCTION_NAME(type, type ##_dup)
...
#define DEFINE_APR_ARRAY_TO_ARRAY(return_type, name, conv, amp, type, ctx) return_type name(const apr_array_header_t *apr_ary) { VALUE ary = rb_ary_new(); int i; for (i = 0; i < apr_ary->nelts; i++) { rb_ary_push(ary, conv((void *)amp(APR_ARRAY_IDX(apr_ary, i, type)), ctx)); } return ary; }
