...
#define HOOKS_ENVIRONMENT_TEXT "#The hook program typically does not inherit the environment of" NL "#its parent process. For example, a common problem is for the" NL "#PATH environment variable to not be set to its usual value, so" NL "#that subprograms fail to launch unless invoked via absolute path." NL "#If you're having unexpected problems with a hook program, the" NL "#culprit may be unusual (or missing) environment variables." NL
#define PREWRITTEN_HOOKS_TEXT "#For more examples and pre-written hooks, see those in" NL "#the Subversion repository at" NL "#http://svn.collab.net/repos/svn/trunk/tools/hook-scripts/ and" NL "#http://svn.collab.net/repos/svn/trunk/contrib/hook-scripts/" NL
static svn_error_t *
