...
#define TEST_MANY(args, expect) result = svn_path_join_many args ; if (strcmp(result, expect) != 0) return svn_error_createf(SVN_ERR_TEST_FAILED, NULL, "svn_path_join_many" #args " returns \"%s\". " "expected \"%s\"", result, expect); else
